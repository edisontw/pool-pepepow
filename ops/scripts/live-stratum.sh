...
disk_status_service() {
  ...
  df -h /
  echo
  echo "runtime_dir_size:"
  du -sh "${RUNTIME_DIR}" 2>/dev/null || true
  echo
  echo "runtime_files_over_100mb:"
  find "${RUNTIME_DIR}" -maxdepth 1 -type f -size +100M -printf '%s %p\n' 2>/dev/null | sort -nr | head -20 || true
}

rotate_runtime_logs_service() {
  ensure_runtime_dir
  python3 "${SCRIPT_DIR}/rotate_runtime_logs.py" --runtime-dir "${RUNTIME_DIR}"
}
...
for raw_line in selected:
    ...
    print(f"candidate_block_hash: {payload.get('candidate_block_hash') or payload.get('candidateBlockHash')}")
    ...
    print(f"localComputedHash: {payload.get('localComputedHash')}")
    print(f"localComputedHashReversed: {payload.get('localComputedHashReversed')}")
    print(f"blockTargetUsed: {payload.get('blockTargetUsed')}")
    print(f"shareTargetUsed: {payload.get('shareTargetUsed')}")
    print(f"meetsBlockTarget: {payload.get('meetsBlockTarget')}")
    print(f"meetsShareTarget: {payload.get('meetsShareTarget')}")
    print(f"candidateHashInt: {payload.get('candidateHashInt') or payload.get('localComputedHashInt')}")
    print(f"blockTargetInt: {payload.get('blockTargetInt')}")
    print(f"candidateComparisonOrder: {payload.get('candidateComparisonOrder')}")
    print(f"bits: {payload.get('bits') or payload.get('nbits')}")
    target_context = payload.get('targetContext') or {}
    print(f"targetContext.height: {target_context.get('height') if isinstance(target_context, dict) else None}")
    print(f"targetContext.curtime: {target_context.get('curtime') if isinstance(target_context, dict) else None}")
    print(f"jobStatus: {payload.get('jobStatus')}")
    print(f"candidate_freshness: {payload.get('candidate_freshness')}")
    print(f"candidateFreshnessStatus: {payload.get('candidateFreshnessStatus')}")
...
accepted_candidates_service() {
  ensure_runtime_dir
  local output_json
  output_json="${RUNTIME_DIR}/accepted-candidates.json"
  python3 "${SCRIPT_DIR}/track_accepted_candidates.py" "${CANDIDATE_OUTCOME_EVENT_LOG}" "${output_json}"
}
...
payout_review_check_service() {
  ...
  local carry_file="${RUNTIME_DIR}/payout-carry-snapshot.json"
  local payments_file="${RUNTIME_DIR}/payments-snapshot.json"
  python3 "${SCRIPT_DIR}/payout_helper.py" payout-review-check \
    --candidates "${payout_file}" \
    --carry-snapshot "${carry_file}" \
    --payments-snapshot "${payments_file}"
}
...
pool_wallet_watchdog_service() {
  ensure_runtime_dir
  python3 "${SCRIPT_DIR}/pool_wallet_monitor.py" watchdog "$@"
}

pool_health_service() {
  ensure_runtime_dir
  python3 "${SCRIPT_DIR}/pool_health_summary.py" "$@"
}
...
solo_auto_payout_once_service() {
  ensure_runtime_dir
  local solo_runtime="${RUNTIME_DIR}/solo"
  if [[ ! -d "${solo_runtime}" ]]; then
    echo "solo runtime directory missing: ${solo_runtime}" >&2
    return 1
  fi
  solo_payout_refresh_service
...
refresh_payment_confirmations_service() {
  ensure_runtime_dir
  python3 "${SCRIPT_DIR}/payout_helper.py" refresh-payment-confirmations \
    --actions-log "${RUNTIME_DIR}/payment-actions.jsonl" \
    --snapshot "${RUNTIME_DIR}/payments-snapshot.json"
}
...
payout_carry_service() {
  ensure_runtime_dir
  local input_payout_candidates output_carry_snapshot
  input_payout_candidates="${RUNTIME_DIR}/payout-candidates.json"
  output_carry_snapshot="${RUNTIME_DIR}/payout-carry-snapshot.json"
  python3 "${SCRIPT_DIR}/payout_helper.py" build-carry-snapshot \
    --candidates "${input_payout_candidates}" \
    --snapshot "${output_carry_snapshot}"
}

payout_carry_audit_service() {
  ensure_runtime_dir
  local input_candidates input_carry input_payments
  input_candidates="${RUNTIME_DIR}/payout-candidates.json"
  input_carry="${RUNTIME_DIR}/payout-carry-snapshot.json"
  input_payments="${RUNTIME_DIR}/payments-snapshot.json"
  python3 "${SCRIPT_DIR}/payout_helper.py" audit-carry-consistency \
...
payout_wallet_dry_run_service() {
  ...
  local output_file="${RUNTIME_DIR}/payout-wallet-dry-run.json"
